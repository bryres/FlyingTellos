...
class Tello:
    ...
    def send_command_with_return(self, command):
        ...
        if diff < self.TIME_BTW_COMMANDS:
            duration = self.TIME_BTW_COMMANDS - diff
            self.write_log_message("Sleeping: %.2f" % duration)
        ...
        # Loop until the drone responses, up to self.RESPONSE_TIMEOUT seconds
        while self.response is None:
            if time.time() - timestamp > self.RESPONSE_TIMEOUT:
                self.write_log_message('Timeout exceed on command ' + command)
                raise Exception('Timeout exceed on command ' + command)

            time.sleep(.05)

        self.write_log_message('Response: ' + str(self.response))
    ...
    @accepts(direction=str, x=(int, float))
    def move(self, direction, x):
        ...
        Returns:
            bool: True for successful, False for unsuccessful
        """
        return self.send_control_command("%s %.0f" % (direction,x))
    ...
    def send_rc_control(self, left_right_velocity, forward_backward_velocity, up_down_velocity, yaw_velocity):
        ...
        Returns:
            bool: True for successful, False for unsuccessful
        """
        if time.time() - self.last_rc_control_sent < self.TIME_BTW_RC_CONTROL_COMMANDS:
            pass
        else:
            self.last_rc_control_sent = time.time()
    ...
    def get_distance_tof(self):
        ...
        Returns:
            ...
            float: 30-1000
        """
        dist = self.send_read_command('tof?')
        dist = float(dist.split("mm")[0])/10
